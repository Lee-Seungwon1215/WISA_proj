...
6. Evidence: a real PQC corpus where **every layer pulls its weight**, every
   verdict class is grounded, and support tables show the ablation and ML-DSA
   attribution details.
...
| 5 | Results | 3.0 | **Coverage + verdict corpus are the headline tables.** Walk coverage rows; foreground KyberSlash (ct PASS/asm FAIL) and ML-DSA (default-deny). Use ablation, ML-DSA attribution, and dudect as support. |
...
| 7 | Conclusion | 0.5 | Restate: layer-justified in this corpus + validated on concrete targets + honest (default-deny). |
...
- **Fig 1** — pipeline diagram: yaml config -> {build matrix -> Valgrind,
  asm-scan, dudect} -> triage taxonomy (default-deny) -> verdict_class. ✅ present;
  polish only if print output looks cramped.
- **Table 1** — per-feature single-coverage. ✅ ready.
- **Table 2** — generated ablation/miss support. ✅ ready.
- **Table 3** — verdict-class corpus, 7 rows. ✅ ready.
- **Table 4** — generated ML-DSA attribution split. ✅ ready.
- **Table 5** — dudect appendix, 2 rows. ✅ ready.

## What's done vs what's left

**Done (evidence is airtight):** all 5 feature single-coverage cases captured;
corpus 7 rows / 5 classes; ablation support; ML-DSA attribution split; dudect
