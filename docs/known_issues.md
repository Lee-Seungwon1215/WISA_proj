...
**Status: Interim doc note RESOLVED in Bundle H1.** README §"ct-leak
모드의 본질적 한계"에 FO-fallback 미커버 단락 명시. Primary fix (신규
`leak_target: fo` 모드 — U2 #1)는 timing_kem.c.j2 세 번째 branch 작업이라
follow-up으로 별도 남김. R1 Option B (randombytes interpose)와 같은 호흡에
처리될 수 있는 후보.
