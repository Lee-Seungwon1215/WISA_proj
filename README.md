...
세 단계 다 통과하면 `CLEAN`, 부분 실패면 `STRUCTURAL_LEAK`/`SUSPECT`/`RISKY`, 양쪽 다 실패면 `CRITICAL`.

> **⚠ PASS는 "constant-time"이 아니다.** verdict CLEAN은 *"이 프레임워크가 돌린 3개 layer가 leak을 못 잡았다"*는 뜻이지 *"이 코드는 안전하다"*가 아니다. 우리가 안 보는 것: power side-channel, EM, fault injection, formal verification, KyberSlash 류 operand-timing(div/mul) 누수, 알고리즘별 adversarial-input test. PASS를 받았다고 다른 분석 도구(masking analysis, power-trace, 정적 검증, 알고리즘별 adversarial test)를 건너뛰지 말 것. 자세한 한계는 §"Limitations & recommended environment" 참고.
...
[3] dudect — Statistical Timing
    ...
    "secret 값에 따라 실행 시간 분포가 통계적으로 다른가?"
         ↓
[verdict] CLEAN / STRUCTURAL_LEAK / SUSPECT / RISKY / CRITICAL
```
...
**KyberSlash는 별도 축이다.** KyberSlash1/2의 핵심은 정상 PKE dec/enc 경로에서
비밀값이 division operand로 들어가고, 일부 컴파일러/옵션/CPU에서 그 division
latency가 입력 의존이 되는 문제다. 그래서 `leak_target: ct` 랜덤샘플 PASS로
부재를 말할 수 없고, 현재 CT-KAT의 더 직접적인 blind spot은 아래
`division/multiplication latency 미검출` 항목이다. 검출하려면 패치드 Valgrind,
objdump/Capstone 기반 operand scan, 또는 알고리즘별 adversarial test가 필요하다.
...
**exit 코드는 default-deny**: `robust`/`accepted-variable-time` 만 `0`, 나머지
(미triage·불완전 스캔 포함) 전부 `2` — 즉 `screen && deploy`는 사람이 triage
하기 전 새 타깃을 통과시키지 않는다. `ct` 섹션 필수, `valgrind`/컴파일러/`objdump`
누락 시 toolchain 에러로 `2`. Valgrind 필요 → Linux/Docker 전용, 단일 서브커맨드보다
무겁다.
...
| FAIL | PASS / NONE | **STRUCTURAL_LEAK** | 구조적 finding 확정 — 통계는 안 잡힘. **"LOW"가 아니라 "구조적 leak"이라는 이름이 정직함** (Bundle I U6 Option A 이전 라벨: `LOW_RISK`) |
