...
\newcommand{\NativeAbstractSentence}{Native results are pending.}
\newcommand{\NativeResultSummary}{Native results are pending.}
...
\input{generated/native_summary.tex}
...
\input{generated/static_results.tex}
...
\input{generated/native_results.tex}

When the result block is complete, every value in it is generated from
\path{paper_native_analysis.json}. No preliminary statistic is copied from
the legacy WISA manuscript, and no partial component is used to fill an empty
row.
...
The complete V10 result in Section~\ref{sec:native-results} adds host-scoped
physical evidence without changing those claim boundaries.
\else
...
\dataavailability{Source code, frozen protocols, and premeasurement artifacts are available at \url{https://github.com/Lee-Seungwon1215/WISA_proj}. The complete V10 raw and derived artifact bundle will be deposited with a stable identifier after integrity validation; that identifier must be added before submission.}
