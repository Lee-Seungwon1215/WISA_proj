...
% References were source-checked on 2026-06-09; do a final eyeball pass before
...
% TODO(author): replace placeholder author / affiliation before submission.
...
matrix feeding three checkers (Valgrind/Memcheck, asm-scan, dudect); a
registry-backed triage policy reduces the results to a \texttt{verdict\_class}.}
\label{fig:pipeline}
\end{figure}
...
\begin{table}[htbp]
\centering
\caption{Per-feature coverage map: each checker or triage rule contributes a
signal that the others do not provide in this corpus.}
\label{tab:coverage}
...
triage policy & ambiguous structural FAIL &
ML-DSA-65 (\texttt{sign}) &
\texttt{needs-analysis}: held for review\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{ML-DSA-65 attribution stress test: debug cells show registered
accepted-variable-time functions; optimized cells surface coarser unregistered
names that keep the row at \texttt{needs-analysis}.}
\label{tab:mldsa}
\footnotesize
\setlength{\tabcolsep}{4pt}
