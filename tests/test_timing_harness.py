...
def test_monotonic_template_uses_clock_gettime():
    ...
    assert out.index("#define _POSIX_C_SOURCE 200809L") < out.index("#include <time.h>")
    assert "#ifndef CLOCK_MONOTONIC_RAW" in out
...
def test_timing_templates_accept_runtime_seed_override(template, context):
    ...
    if template == "generic":
        ...
        assert "ctkat_parse_options" in out
        assert "ctkat_prng_init(options.seed)" in out
...
def test_kem_class_label_uses_high_bits():
    ...
    assert "ctkat_label_next() >> 32" in out
...
def test_kem_captures_dec_return_with_ctkat_use():
    ...
    assert "int dec_rc" in out
    assert "CTKAT_USE(dec_rc)" in out
...
def test_kem_sinks_output_shared_secret_after_timed_window():
    ...
    for lt in ("sk", "ct", "fo"):
        ...
        assert out.index("t1 = ctkat_now(&aux1)") < out.index("volatile uint64_t sink"), lt


def test_kem_underflow_clamp_present():
    out = render_timing_harness("kem", _kem_ctx())
    assert "if (t1 < t0)" in out
    assert "CTKAT_DROP_CLOCK" in out


def test_kem_placebo_normalizes_the_common_work_buffers():
    out = render_timing_harness("kem", _kem_ctx())
    assert "sk_placebo" not in out
    assert "ct_placebo" not in out
    assert "memcpy(sk_work, sk_fixed, CTKAT_SK_BYTES)" in out
    assert "memcpy(ct_work, ct_fixed, CTKAT_CT_BYTES)" in out
    assert out.count("crypto_kem_dec(ss_work, ct_work, sk_work)") == 2  # warmup + measured


def test_kem_monotonic_exposes_posix_clock_before_headers():
    out = render_timing_harness("kem", _kem_ctx(clock="monotonic"))
    assert out.index("#define _POSIX_C_SOURCE 200809L") < out.index("#include <time.h>")
    assert "#ifndef CLOCK_MONOTONIC_RAW" in out
...
def test_kem_default_emits_sk_leak_mode():
    # Default sk axis pre-generates both key/ciphertext pools.
    out = render_timing_harness("kem", _kem_ctx())
    assert "pool0_sk" in out
...
def test_kem_ct_leak_mode_uses_fixed_ct():
    ...
    # Both secret-key pools receive the same fixed key.
    assert "CTKAT_SLOT(pool1_sk, p, CTKAT_SK_BYTES), sk_fixed" in out


def test_kem_ct_leak_class_branch_uses_ct_fixed_vs_random():
    out = render_timing_harness("kem", _kem_ctx(leak_target="ct"))
    # Class 0 pool copies the fixed ct; class 1 pool is filled by enc() before
    # the measurement loop.
    assert "CTKAT_SLOT(pool0_ct, p, CTKAT_CT_BYTES), ct_fixed" in out
    assert "crypto_kem_enc(ct_tmp, ss_tmp, pk_fixed)" in out
...
def test_kem_fo_mode_uses_valid_ct_for_class_0():
    ...
    assert "crypto_kem_enc(ct_tmp, ss_tmp, pk_fixed)" in out
    assert "CTKAT_SLOT(pool0_ct, p, CTKAT_CT_BYTES), ct_tmp" in out
...
def test_kem_sk_leak_uses_valid_ct_for_both_classes(_kem_ctx=_kem_ctx):
    ...
    # Both pool sides contain valid ct paired with the corresponding sk.
    assert "crypto_kem_enc(ct_fixed, ss_tmp, pk_fixed)" in code
    assert "crypto_kem_enc(ct_tmp, ss_tmp, pk_tmp)" in code
    # The old rand_bytes(ct, ...) measurement step must be gone.
    assert "ctkat_rand_bytes(ct_work" not in code
...
def _extract_warmup_block(code: str) -> str:
    ...
    measure_idx = code.find("i < options.measurements")
...
def test_kem_ct_leak_warmup_uses_fixed_valid_ct(_kem_ctx=_kem_ctx):
    ...
    code = _strip_comments(out)
    warmup_block = _extract_warmup_block(code)
    assert "pool0_ct" in warmup_block
    assert "pool1_ct" in warmup_block
    assert "crypto_kem_keypair" not in warmup_block
...
def test_kem_v2_controls_common_buffers_and_aux_filter():
    out = render_timing_harness("kem", _kem_ctx(clock="rdtsc"))
...
def test_sign_keypair_outside_and_signature_inside_timed_window():
    ...
    t0 = out.index("t0 = ctkat_now(&aux0)")
    t1 = out.index("t1 = ctkat_now(&aux1)")
    sig_call = out.index("sig_rc = TEST_crypto_sign_signature")
    assert out.index("siglen = 0", out.index("i < options.measurements")) < t0
    assert t0 < sig_call < t1


def test_sign_captures_return_with_ctkat_use():
    out = render_timing_harness("sign", _sign_ctx())
    assert "int sig_rc" in out
    assert "CTKAT_USE(sig_rc)" in out
...
def test_sign_sinks_signature_after_timed_window():
    ...
    out = render_timing_harness("sign", _sign_ctx())
    assert "volatile uint64_t sink" in out
    assert "sig_work[j]" in out
    assert out.index("t1 = ctkat_now(&aux1)") < out.index("volatile uint64_t sink")


def test_sign_pool_setup_not_per_iteration_keygen():
    out = render_timing_harness("sign", _sign_ctx())
...
def test_sign_fixed_vs_fresh_sk_axis():
    ...
    assert "pool0_sk" in out
    assert "pool1_sk" in out
    assert "TEST_crypto_sign_keypair(pk_tmp, sk_tmp)" in out
...
def test_sign_reports_output_length_and_full_api_scope():
    out = render_timing_harness("sign", _sign_ctx())
...
int TEST_crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    for (size_t i = 0; i < 16; i++) sk[i] = (uint8_t)(i + counter);
    counter++;
    memcpy(pk, sk, 16);
    return 0;
}
...
""",
    ...
    for mode, effect in (
        ...
        samples = run_timing_harness(
            ...
            mode=mode,
            effect_ticks=effect,
            measurements_override=16,
        )
        assert samples.raw_n_total == 16
