...
/* A trivial 256-entry "S-box". The actual values don't matter for the
 * purposes of constant-time analysis; what matters is the access pattern. */
static const uint8_t sbox[256] = {
    /* identity table — values are unimportant for this demo */
#define I(x) (uint8_t)(x)
...
/*
 * Address calculation `sbox + secret[i]` depends on a secret-tainted value.
 * Valgrind reports "Use of uninitialised value of size 1" for the load.
 * The stack-frame function name contains "lookup" → ctkat's heuristic
 * promotes the finding to SECRET_DEPENDENT_MEMORY_ACCESS.
 */
...
/*
 * Index is the loop counter (public), so the address is data-independent.
 * The secret is only XOR-mixed into the output value, not used as an
 * address. Valgrind should not flag this.
 */
void safe_lookup(const uint8_t *secret, uint8_t *out, size_t n) {
    for (size_t i = 0; i < n; i++) {
        out[i] = (uint8_t)(sbox[i & 0xff] ^ secret[i]);
    }
}
