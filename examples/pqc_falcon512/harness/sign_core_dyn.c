...
static int prepare_key(int8_t f[N], int8_t g[N], int8_t F[N], int8_t G[N]) {
    ...
    return PQCLEAN_FALCON512_CLEAN_complete_private(G, f, g, F, LOGN, tmp.b) ? 0 : -1;
}
...
int main(void) {
    puts("CTKAT-HARNESS-RAN: sign_core_dyn");
    ...
    VALGRIND_MAKE_MEM_UNDEFINED(g, sizeof(g));
    VALGRIND_MAKE_MEM_UNDEFINED(F, sizeof(F));
    VALGRIND_MAKE_MEM_UNDEFINED(G, sizeof(G));

    PQCLEAN_FALCON512_CLEAN_sign_dyn(sig, &rng, f, g, F, G, hm, LOGN, tmp.b);

    VALGRIND_MAKE_MEM_DEFINED(f, sizeof(f));
    VALGRIND_MAKE_MEM_DEFINED(g, sizeof(g));
    VALGRIND_MAKE_MEM_DEFINED(F, sizeof(F));
    VALGRIND_MAKE_MEM_DEFINED(G, sizeof(G));
